...
describe("IterablePromise join operators", () => {
    describe("join", () => {
        ...
        it("joins multiple inner records to one outer record", async () => {
            let xs = await IterablePromiseImpl.from([1, 2, 2, 3, 4, 5])
            ...
            xs = await IterablePromiseImpl.from([1, 2, 2])
                .join([1, 2, 2, 3, 4, 5], x => x, x => x, (x, y) => [x, y])
                .toArray();
...
describe("IterablePromise ordering operators", () => {
    ...
    describe("orderBy", () => {
        it("correctly sorts number keys", async () => {
            ...
            xs = await IterablePromiseImpl.from([2, 3, 1, 14])
                .orderBy(x => x)
                .toArray();
        ...
        it("lexically sorts string keys", async () => {
            const xs = await IterablePromiseImpl.from([2, 3, 1, 14])
                .orderBy(x => x.toString())
                .toArray();
            assert.deepEqual(xs, [1, 14, 2, 3]);
        });

        it("rejects if key function returns something other than number | string", async () => {
            assert.rejects(IterablePromiseImpl.from([2, 3, 1, 14]).orderBy(() => <any>[2]).toArray);
            assert.rejects(
                IterablePromiseImpl.from([2, 3, 1, 14]).orderBy(() => {
                    return <number>{};
                }).toArray,
            );
        });

        it("rejects if key function does not return only keys of one type", async () => {
            assert.rejects(
                IterablePromiseImpl.from([2, 3, 1, 14]).orderBy(x => {
                    if (x === 2) {
                        return "2";
                    }
                    return x;
...
describe("IterablePromise aggregate operators", () => {
    ...
    describe("sum", () => {
        it("returns 0 for empty array", async () => {
            const res = await IterablePromiseImpl.from<number>([]).sum();
            assert.deepEqual(res, 0);
        });

        it("correctly calculates the sum of array of numbers", async () => {
            const res = await IterablePromiseImpl.from([1, 2, 3, 4, 5]).sum();
            assert.deepEqual(res, 15);
        });

        it("throws when summing non-numbers", async () => {
            assert.rejects(IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).sum);
        });

        it("correctly calculates the sum using key func", async () => {
            const res = await IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).sum(
                x => x.foo,
            );
    ...
    describe("min", () => {
        it("throws for empty lists", async () => {
            assert.rejects(IterablePromiseImpl.from<number>([]).min);
        });

        it("correctly finds min for array of numbers", async () => {
            let res = await IterablePromiseImpl.from([1]).min();
            assert.deepEqual(res, 1);

            res = await IterablePromiseImpl.from([1, -1]).min();
            assert.deepEqual(res, -1);
        });

        it("throws when finding min of non-numbers", async () => {
            assert.rejects(IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).min);
        });

        it("correctly calculates the min using key func", async () => {
            const res = await IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).min(
                x => x.foo,
            );
            assert.deepEqual(res, 1);
        });
    ...
    describe("max", () => {
        it("throws for empty lists", async () => {
            assert.rejects(IterablePromiseImpl.from<number>([]).max);
        });

        it("correctly finds max for array of numbers", async () => {
            let res = await IterablePromiseImpl.from([1]).max();
            assert.deepEqual(res, 1);

            res = await IterablePromiseImpl.from([1, -1]).max();
            assert.deepEqual(res, 1);
        });

        it("throws when finding max of non-numbers", async () => {
            assert.rejects(IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).max);
        });

        it("correctly calculates the max using key func", async () => {
            const res = await IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).max(
                x => x.foo,
            );
    ...
    describe("average", () => {
        it("throws for empty lists", async () => {
            assert.rejects(IterablePromiseImpl.from<number>([]).average);
        });

        it("correctly finds average for array of numbers", async () => {
            let res = await IterablePromiseImpl.from([1]).average();
            assert.deepEqual(res, 1);

            res = await IterablePromiseImpl.from([1, -1]).average();
            assert.deepEqual(res, 0);
        });

        it("throws when finding average of non-numbers", async () => {
            assert.rejects(IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).average);
        });

        it("correctly calculates the average using key func", async () => {
            const res = await IterablePromiseImpl.from([
                { foo: 1 },
                { foo: 2 },
                { foo: 3 },
            ]).average(x => x.foo);
    ...
    describe("aggregate", () => {
        it("throws for empty lists", async () => {
            const res = await IterablePromiseImpl.from<number>([]).aggregate(0, acc => acc + 13);
            assert.deepEqual(res, 0);
        });

        it("aggregates (+) over array of numbers", async () => {
            let res = await IterablePromiseImpl.from([1]).aggregate(0, (acc, i) => acc + i);
            assert.deepEqual(res, 1);

            res = await IterablePromiseImpl.from([1, -1]).aggregate(0, (acc, i) => acc + i);
            assert.deepEqual(res, 0);

            res = await IterablePromiseImpl.from([{ foo: 1 }, { foo: 2 }, { foo: 3 }]).aggregate(
                0,
                (acc, o) => acc + o.foo,
            );
            assert.deepEqual(res, 6);
